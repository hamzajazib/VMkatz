...
/// Multiple patterns for different Windows 10/11 builds.
...
pub static WDIGEST_LOGON_SESSION_PATTERNS: &[&[u8]] = &[
    // Win10 1607+: CMP RBX,RCX; JE
    &[0x48, 0x3B, 0xD9, 0x74],
    // Win10 older: CMP RCX,RBX; JE
    &[0x48, 0x3B, 0xCB, 0x74],
    // Win10 1809+: LEA pattern in SpAcceptCredentials
    &[0x48, 0x3B, 0xD9, 0x0F, 0x84],
    // Alternate: CMP RCX,RBX; JE (near)
...
pub static TSPKG_LOGON_SESSION_PATTERNS: &[&[u8]] = &[
...
pub fn resolve_rip_relative(vmem: &impl VirtualMemory, code_addr: u64, disp_offset: i64) -> Result<u64> {
    ...
    log::debug!(
        ...
        code_addr, disp_offset, disp_addr, displacement, target
    );
    Ok(target)
}
